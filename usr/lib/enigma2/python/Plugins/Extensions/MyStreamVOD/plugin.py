# Embedded file name: /usr/lib/enigma2/python/Plugins/Extensions/MyStreamVOD/plugin.py
import nVars
...
import re
import urllib2
from urllib2 import Request, urlopen, URLError, HTTPError, quote as urllib2_quote, unquote as urllib2_unquote
...
sys.path.append('/usr/lib/enigma2/python/Plugins/Extensions/MyStreamVOD/Moduls')
dirPathM = '/usr/lib/enigma2/python/Plugins/Extensions/MyStreamVOD/Moduls/'
modulsF = '/usr/lib/enigma2/python/Plugins/Extensions/MyStreamVOD/Moduls/nStreamModuls.py'
if os.path.exists(modulsF) == False:
    os.path.dirname(modulsF)
modulsF = open(modulsF, 'w')
modulsF.write('class html_parser_moduls:\n')
modulsF.write('\tdef __init__(self):\n')
modulsF.write('\t\tself.video_list = []\n')
modulsF.write("\t\tself.next_page_url = ''\n")
modulsF.write("\t\tself.next_page_text = ''\n")
modulsF.write("\t\tself.prev_page_url = ''\n")
modulsF.write("\t\tself.prev_page_text = ''\n")
modulsF.write("\t\tself.search_text = ''\n")
modulsF.write("\t\tself.search_on = ''\n")
modulsF.write("\t\tself.active_site_url = ''\n")
modulsF.write("\t\tself.playlistname = ''\n")
modulsF.write("\t\tself.playlist_cat_name = ''\n")
modulsF.write("\t\tself.kino_title = ''\n")
modulsF.write("\t\tself.category_back_url = ''\n")
modulsF.write("\t\tself.error = ''\n")
modulsF.write('\tdef get_list(self, url):\n')

def creat(File):
    modulsF.write('\t\tfrom ' + File + ' import ' + 'html_parser_' + File1[1] + '\n')
    modulsF.write("\t\tif(url.find('%s')>-1):\n" % File1[1])
    modulsF.write('\t\t\t' + File1[1] + ' = html_parser_%s()\n' % File1[1])
    modulsF.write('\t\t\t' + File1[1] + '.get_list(url)\n')
    modulsF.write('\t\t\tself.video_list = %s.video_list\n' % File1[1])
    modulsF.write('\t\t\tself.next_page_url = %s.next_page_url\n' % File1[1])
    modulsF.write('\t\t\tself.next_page_text = %s.next_page_text\n' % File1[1])
    modulsF.write('\t\t\tself.prev_page_url = %s.prev_page_url\n' % File1[1])
    modulsF.write('\t\t\tself.prev_page_text = %s.prev_page_text\n' % File1[1])
    modulsF.write('\t\t\tself.search_text = %s.search_text\n' % File1[1])
    modulsF.write('\t\t\tself.search_on = %s.search_on\n' % File1[1])
    modulsF.write('\t\t\tself.active_site_url = %s.active_site_url\n' % File1[1])
    modulsF.write('\t\t\tself.playlistname = %s.playlistname\n' % File1[1])
    modulsF.write('\t\t\tself.playlist_cat_name = %s.playlist_cat_name\n' % File1[1])
    modulsF.write('\t\t\tself.kino_title = %s.kino_title\n' % File1[1])
    modulsF.write('\t\t\tself.category_back_url = %s.category_back_url\n' % File1[1])
    modulsF.write('\t\t\tself.error = %s.error\n' % File1[1])


dirTree = os.walk(dirPathM)
for dirPathM, dirNames, Files in dirTree:
    for File in Files:
        if File[-3:] == 'pyo' or File[-2:] == 'py':
            File = File.replace('.pyo', '')
            File = File.replace('.py', '')
            if File != 'nStreamModuls':
                print 'Creating %s\n' % File
                File1 = File.split('_')
                print File1[1]
                creat(File)

modulsF.write('\t\treturn self.video_list\n')
modulsF.close()
from nStreamModuls import html_parser_moduls
...
dirPath = '/usr/lib/enigma2/python/Plugins/Extensions/MyStreamVOD/m3uDir/'
m3u = '/usr/lib/enigma2/python/Plugins/Extensions/MyStreamVOD/m3u.xml'
if os.path.exists(m3u) == False:
    os.path.dirname(m3u)
m3u = open(m3u, 'w')
m3u.write('<?xml version="1.0" encoding="UTF-8" ?>\n')
m3u.write('<items>\n')
m3u.write('<playlist_name>M3U list</playlist_name>\n')

def creat(File):
    m3u.write('\t<channel>\n')
    m3u.write('\t\t\t<title>%s</title>\n' % File)
    m3u.write('\t\t\t<playlist_url><![CDATA[nStreamModul@' + File + '@m3u@' + File.replace('m3u', '') + ']]></playlist_url>\n')
    m3u.write('\t</channel>\n')


dirTree = os.walk(dirPath)
for dirPath, dirNames, Files in dirTree:
    for File in Files:
        if File[-3:] == 'm3u':
            FullPath = os.path.join(os.path.abspath(dirPath), File)
            creat(File)
...
class iptv_streamse():
    ...
    def read_config(self):
        try:
            tree = ElementTree()
            xml = tree.parse('/usr/lib/enigma2/python/Plugins/Extensions/MyStreamVOD/nStreamConfig.xml')
    ...
    def get_list(self, url = None, send_mac = True):
        ...
        try:
            if url == 'playlist_history':
                ...
                tree = ElementTree()
                xml = tree.parse('/usr/lib/enigma2/python/Plugins/Extensions/MyStreamVOD/nStreamVOD.xml')
            elif url == 'favorites':
                tree = ElementTree()
                if self.server_oki:
                    xml = tree.parse('/usr/lib/enigma2/python/Plugins/Extensions/MyStreamVOD/nStreamFavorites.xml')
                ...
                if xml == 'Fake':
                    tree = ElementTree()
                    xml = tree.parse('/usr/lib/enigma2/python/Plugins/Extensions/MyStreamVOD/tmp.xml')
            else:
                tree = ElementTree()
                xml = tree.parse('/usr/lib/enigma2/python/Plugins/Extensions/MyStreamVOD/' + url)
            if xml:
                self.next_page_url = ''
                self.next_page_text = ''
                self.prev_page_url = ''
                ...
                search_text_element = 'search'
                self.search_text = 'Buscar'
                chan_counter = 0
                for channel in xml.findall('channel'):
                    chan_counter = chan_counter + 1
                    name = channel.findtext('title').encode('utf-8')
                    if name.find('img src') > -1:
                        colo = re.findall('<img src="(.*)".*>', name)
                        if colo:
                            name = name.replace(colo[0], '')
                    if name.find('color') > -1:
                        colo = re.findall('<span style="color(.*)">', name)
                        if colo:
                            name = name.replace(colo[0], '')
                        colo = re.findall('<font color="(.*)">', name)
                        if colo:
                            name = name.replace(colo[0], '')
                    name = name.replace('<img src="', '')
                    name = name.replace('"/>', '')
                    name = name.replace('</a>', '')
                    name = name.replace('<br />', '')
                    name = name.replace('/>', '')
                    name = name.replace('<span style="color', '')
                    name = name.replace('">', '')
                    name = name.replace('</span>', '')
                    name = name.replace('<font color="', '')
                    ...
                    if description != None:
                        ...
                        description4playlist_html = 'no_description'
    ...
    def _request(self, url, mac = None):
        ...
        try:
            req = urllib2.Request(url, None, {'User-agent': 'Mozilla/5.0 MyStreamVOD 0.1',
             'Connection': 'Close'})
            xmlstream = urllib2.urlopen(req, timeout=NTIMEOUT).read()
            tmp = '/usr/lib/enigma2/python/Plugins/Extensions/MyStreamVOD/tmp.xml'
            ...
            if xmlstream.splitlines()[0].find('#EXTM3U') > -1:
                ...
                tmp.write('<items>\n')
                tmp.write('<playlist_name>M3U</playlist_name>\n')
                while stri < len(xmlstream.splitlines()[1:]):
                    stri = stri + 1
                    line = xmlstream.splitlines()[stri]
                    if line.find('#EXTINF:') > -1:
                        tmp.write('\t<channel>\n')
                        tmp.write('\t\t\t<title>%s</title>\n' % line.replace('#EXTINF:', '').replace('0', '').replace('1', '').replace(',', ''))
                    if line != '' and line.find('#EXTINF:') == -1:
                        tmp.write('\t\t\t<stream_url><![CDATA[%s]]></stream_url>\n' % line)
                        tmp.write('\t</channel>\n\n\n')

                tmp.write('</items>\n')
                tmp.close()
                res = 'Fake'
            if xmlstream.splitlines()[1].find('<?xml') > -1 or xmlstream.splitlines()[0].find('<items>') > -1 and xmlstream.splitlines()[0].find('<?xml') == -1 or xmlstream.splitlines()[0].find('      ') > -1:
                tmp = open(tmp, 'w')
                if xmlstream.splitlines()[0].find('<items>') > -1:
                    tmp.write('<?xml version="1.0" encoding="UTF-8" ?>\n')
                    tmp.write(xmlstream.splitlines()[0])
                if xmlstream.splitlines()[0].find('      ') > -1:
                    tmp.write('<?xml version="1.0" encoding="UTF-8" ?>\n')
                while stri < len(xmlstream.splitlines()[1:]):
                    stri = stri + 1
                    line = xmlstream.splitlines()[stri]
    ...
    def write_favorites(self):
        ...
        try:
            fileObj = open('/usr/lib/enigma2/python/Plugins/Extensions/MyStreamVOD/nStreamFavorites.xml', 'w')
            fileObj.write('<?xml version="1.0" encoding="UTF-8" ?>\n')
            fileObj.write('<items>\n')
            fileObj.write('<playlist_name>Favoritos</playlist_name>\n')
...
PLUGIN_PATH = '/usr/lib/enigma2/python/Plugins/Extensions/MyStreamVOD'
...
def Start_iptv_palyer(session, **kwargs):
    ...
    print '\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n'
    print '######################################################################'
    print '#######--------------- START MyStreamVOD v%s ---------------#######' % VERSION
    print '######################################################################'
    HW_INFO = getInfo()
    print HW_INFO['brand']
    print HW_INFO['model']
...
class nPlaylist(Screen):
    ...
    def upgrade(self):
        self.session.open(plugin_upgrade2)
    ...
    def start_favorites(self):
        self.index = 0
        STREAMS.get_list('nStreamFavorites.xml')
        self.update_channellist()
    ...
    def show_all(self):
        try:
            if self.passwd_ok == False:
                ...
                self['version'].setText('v1.2 by nitrogen14\n' + 'mod by <brigada CSC> v2.0 | giclub.tv')
...
def Plugins(**kwargs):
    return [PluginDescriptor(name='MyStreamVOD', description='VOD & IPTV Player', where=PluginDescriptor.WHERE_MENU, fnc=menu), PluginDescriptor(name='MyStreamVOD', description='VOD & IPTV Player', where=PluginDescriptor.WHERE_PLUGINMENU, fnc=Start_iptv_palyer, icon='plugin.png')]
...
class plugin_upgrade2(Screen):
    skin = '\n\t\t<screen position="center,center" size="380,120" title="%s">\n\t\t\t<widget name="list" position="5,5" size="370,115" />\n\t\t</screen>' % _('Update plugin')
    ...
    def showMenu(self):
        list = []
        list.append(_('Upgrade plugin OE1.6'))
        list.append(_('Upgrade plugin OE2.0'))
        list.append(_('Upgrade keymap'))
        self['list'].setList(list)

    def okClicked(self):
        sel = self['list'].getCurrent()
        if sel == _('Upgrade plugin OE1.6'):
            self.OE16()
        if sel == _('Upgrade plugin OE2.0'):
            self.OE20()
        if sel == _('Upgrade keymap'):
            self.Keymap()

    def OE16(self):
        os.system('wget -q http://sashagamliy.pusku.com/nStreamVOD_OE1.6.tar.gz -O /tmp/nStreamVOD_OE1.6.tar.gz')
        if fileExists('/tmp/nStreamVOD_OE1.6.tar.gz'):
            os.system('rm -rf /usr/lib/enigma2/python/Plugins/Extensions/nStreamVOD')
            os.system('tar -xz -f /tmp/nStreamVOD_OE1.6.tar.gz')
            os.system('rm /tmp/nStreamVOD_OE1.6.tar.gz')
        restart = self.session.openWithCallback(self.restart, MessageBox, '%s\n\n%s\n%s' % (_('Your settings will be installed'), _('GUI needs a reboot to apply the changes.'), _('Do you want to restart the GUI now?')), MessageBox.TYPE_YESNO)
        restart.setTitle('%s' % _('Restart GUI now?'))

    def OE20(self):
        os.system('wget -q http://sashagamliy.pusku.com/nStreamVOD_OE2.0.tar.gz -O /tmp/nStreamVOD_OE2.0.tar.gz')
        if fileExists('/tmp/nStreamVOD_OE2.0.tar.gz'):
            os.system('rm -rf /usr/lib/enigma2/python/Plugins/Extensions/nStreamVOD')
            os.system('tar -xz -f /tmp/nStreamVOD_OE2.0.tar.gz')
            os.system('rm /tmp/nStreamVOD_OE2.0.tar.gz')
        restart = self.session.openWithCallback(self.restart, MessageBox, '%s\n\n%s\n%s' % (_('Your settings will be installed'), _('GUI needs a reboot to apply the changes.'), _('Do you want to restart the GUI now?')), MessageBox.TYPE_YESNO)
        restart.setTitle('%s' % _('Restart GUI now?'))

    def Keymap(self):
        os.system('wget -q http://sashagamliy.pusku.com/keymap.tar.gz -O /tmp/keymap.tar.gz')
        if fileExists('/tmp/keymap.tar.gz'):
            os.system('rm -rf /usr/lib/enigma2/python/Plugins/Extensions/MyStreamVOD/keymap.xml')
            os.system('tar -xz -f /tmp/keymap.tar.gz')
            os.system('rm /tmp/keymap.tar.gz')
        restart = self.session.openWithCallback(self.restart, MessageBox, '%s\n\n%s\n%s' % (_('Your settings will be installed'), _('GUI needs a reboot to apply the changes.'), _('Do you want to restart the GUI now?')), MessageBox.TYPE_YESNO)
        restart.setTitle('%s' % _('Restart GUI now?'))
